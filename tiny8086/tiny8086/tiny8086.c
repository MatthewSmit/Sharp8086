...
int main(int argc, char **argv)
{
	...
	FILE* fileOut = fopen("dmp.txt", "w");
	unsigned short lastCS = -1;
	unsigned short lastIP = -1;

	// Instruction execution loop. Terminates if CS:IP = 0:0
	for (; opcode_stream = mem + 16 * regs16[REG_CS] + reg_ip, opcode_stream != mem;)
	{
		if (regs16[REG_CS] != 0xF000 && regs16[REG_CS] != 0xC000 && (regs16[REG_CS] != lastCS || reg_ip != lastIP))
		{
			make_flags();
			fprintf(fileOut, "%04X:%04X - %s\n", regs16[REG_CS], reg_ip, oplist[*opcode_stream]);
			fprintf(fileOut, "AX: %04X, CX: %04X, DX: %04X, BX: %04X, SP: %04X, BP: %04X, SI: %04X, DI: %04X\n", regs16[REG_AX], regs16[REG_CX], regs16[REG_DX], regs16[REG_BX], regs16[REG_SP], regs16[REG_BP], regs16[REG_SI], regs16[REG_DI]);
			fprintf(fileOut, "ES: %04X, CS: %04X, SS: %04X, DS: %04X, FLAGS: %04X\n", regs16[REG_ES], regs16[REG_CS], regs16[REG_SS], regs16[REG_DS], scratch_uint);
		
			lastCS = regs16[REG_CS];
			lastIP = reg_ip;
		}
